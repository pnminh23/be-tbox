...
export const createBooking = async (req, res) => {
  ...
  if (!name_client)
    return res
      .status(400)
      .json({ success: false, message: "Chưa có tên khách hàng" });
  if (!email)
    return res.status(400).json({ success: false, message: "Chưa có email" });
  if (!phone)
    return res
      .status(400)
      .json({ success: false, message: "Chưa có số điện thoại" });
  if (!room)
    return res
      .status(400)
      .json({ success: false, message: "Chưa có mã phòng" });
  if (!date)
    return res
      .status(400)
      .json({ success: false, message: "Chưa có ngày đặt phòng" });
  if (!time_slots)
    return res
      .status(400)
      .json({ success: false, message: "Chưa có khung giờ" });
  ...
  try {
    ...
    if (existingBooking)
      return res
        .status(400)
        .json({ success: false, message: "Đã có người đặt!" });
    ...
    if (!roomInfo.type)
      return res
        .status(400)
        .json({ success: false, message: "Phòng chưa gán loại phòng (type)" });

    const pricePerMinute = roomInfo.type.base_price_per_minute;

    const selectedtimeSlots = await timeSlotModel
      .find({ _id: { $in: time_slots } })
      .sort({ start_time: 1 });
    const totalMinutes = selectedtimeSlots.reduce(
      (acc, slot) => acc + slot.slot_duration,
      0
    );
    ...
    if (combo) {
      ...
      if (!comboInfo)
        return res
          .status(404)
          .json({ success: false, message: "Combo không tồn tại" });
      ...
      if (!matchedType)
        return res
          ...
          .json({
            success: false,
            message: "Combo không áp dụng cho loại phòng này",
          });
    ...
    return res
      .status(200)
      .json({
        message: "Đặt phòng thành công! Email đã được gửi đến bạn",
        data: result,
      });
  } catch (error) {
...
export const getBookedTimeSlotsByRoom = async (req, res) => {
  ...
  if (!room || !date) {
    return res
      .status(400)
      .json({ success: false, message: "Vui lòng cung cấp room và date" });
  }
...
export const getBookedByOrderCode = async (req, res) => {
  ...
  if (!orderCode) {
    return res
      .status(400)
      .json({ success: false, message: "Vui lòng cung cấp orderCode" });
  }
...
export const getBookedByEmail = async (req, res) => {
  ...
  if (!accountId) {
    return res
      .status(400)
      .json({ success: false, message: "Không có tài khoản" });
  }
...
export const getBookedById = async (req, res) => {
  ...
  if (!id_booking) {
    return res
      .status(400)
      .json({ success: false, message: "Vui lòng cung cấp id_booking" });
  }
...
export const getBookedByEmailCurrent = async (req, res) => {
  ...
  try {
    ...
    if (!bookings) {
      return res
        .status(404)
        .json({ success: false, message: "Không tìm thấy s" });
    }
    ...
    if (!bookings || bookings.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Không có đơn đặt nào chưa hoàn thành",
        });
    }
...
export const getBookedByEmailAndMonth = async (req, res) => {
  ...
  if (!month || !year) {
    return res
      ...
      .json({
        success: false,
        message: "Vui lòng cung cấp email, month và year",
      });
  }
  const user = await accountModel.findById({ _id: accountId });
  const email = user.email;
  ...
  try {
    ...
    if (!bookings.length) {
      return res
        ...
        .json({
          success: false,
          message: "Không tìm thấy đơn nào trong tháng",
        });
    }
...
export const editBooking = async (req, res) => {
  ...
  if (!_id) {
    return res
      .status(400)
      .json({ success: false, message: "Vui lòng cung cấp _id của đơn đặt" });
  }
...
export const getBookingsByMonth = async (req, res) => {
  try {
    ...
    if (!month)
      return res
        .status(400)
        .json({ success: false, message: "Vui lòng cung cấp tháng" });
